...
export class DetailComponent implements OnChanges, OnInit, OnDestroy, AfterViewChecked,DoCheck {
    ...
    entityMessge: EntityMessage = { name: undefined, responded: false, color: undefined };
    ...
    get computeListEntitiesAllowedToRespond(): Array<EntityMessage> {
        const entitiesToRespond = new Array<EntityMessage>();
        if (!this.card.entitiesAllowedToRespond) {
            console.log('Card error : no field entitiesAllowedToRespond');
            return entitiesToRespond;
        }


        this.card.entitiesAllowedToRespond.forEach(entity => {
            if (this.childCards.length === 0) {
                entitiesToRespond.push({ name: this.getEntityName(entity), responded: false, color: 'yellow' });
            } else {
                entitiesToRespond.push({ name: this.getEntityName(entity), responded: this.checkEntityAnswered(entity), color: 'yellow' });
            }
        });
    ...
    getEntityName(id: string): string {
        const entity = this.entitiesService.getEntities().find(entity => entity.id === id);
        if (entity) {
         return entity.name;
        }
    }
